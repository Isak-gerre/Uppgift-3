...
if (!isset($postID, $userID) || empty($postID) || empty($userID)) {
    $message = ["message" => "There went something wrong with the keys, check that again"];
    send($message, 400);
}

// 4. Kollar om bilden finns i databasen 
if (!array_key_exists($postID, $posts)){
    $message = ["message" => "Picture dosn't exist at all"];
    send($message, 404);
}

// 5. Kollar om posten finns i den inloggades array av posts
if (array_search($postID, getUserPosts($userID)) === false){
    $message = ["message" => "You can only delete pictures of your own"];
    send($message, 400);
}
...
$message = ["message" => "The picture is deleted"];
send($message);
